...
import { detectPageLayout } from './utils/detectPageLayout';
...
export function injectGridInspector(options: InjectOptions = {}): void {
  ...
  // Detect real page layout BEFORE mounting (while :root is still clean)
  const detected = detectPageLayout(config?.contentSelector);
  const initialConfig: Partial<GridInspectorConfig> = detected
    ? {
        ...config,
        overlay: {
  ...
  currentRoot.render(<GridInspectorApp config={initialConfig} debug={debug} />);
