...
export function useGridSync() {
  const { overlay, layout, brand, theme } = useGridSystem();
    ...
    if (differences.length > 0) {
      emitGridViolation(
        ...
        'medium',
        {
          ...
          description: `Overlay and Layout differ in: ${differences.join(', ')}`
        }
      );
    }
  }, [overlay, layout, brand, theme]); // Re-check when any value changes
}
