  ...
  if (!isValidSpacingToken(value)) {
    ...
    emitGridViolation(
      ...
      'medium',
      {
        ...
        location: context,
        description: `Value ${value}px does not match DSS spacing tokens. Valid tokens: ${VALID_SPACING_TOKENS.join(', ')}`
      }
    );
  }
}
  ...
  if (!isValidColumnCount(columns)) {
    ...
    emitGridViolation(
      ...
      'low',
      {
        ...
        location: 'overlay',
        description: `Column count ${columns} is not a common convention. Standard counts: ${VALID_COLUMN_COUNTS.join(', ')}`
      }
    );
  }
}
