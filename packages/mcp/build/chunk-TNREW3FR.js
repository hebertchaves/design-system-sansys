...
async function checkCompliance(context, ruleType, dssRoot) {
  ...
  const sources = [];
  for (const { label, path: path2 } of [
    { label: "CLAUDE.md", path: claudePath },
    { label: "DSS_CRITERIOS_AVALIACAO_FASE2.md", path: criteriosPath }
  ]) {
    if (existsSync3(path2)) {
      sources.push({ label, path: path2, content: readFileSync4(path2, "utf-8") });
    }
  }
  if (sources.length === 0) {
    return {
...
// src/tools/validateVisualContract.ts
import * as fs from "fs";
import * as path from "path";
var validate_visual_contract_schema = {
...
async function validateVisualContract(args) {
...
var ValidateGridLayoutSchema = z.object({
  ...
  }).optional().describe("Grid overlay configuration (visual grid layer)."),
  layout: z.object({
    gutter: GridSpacingSchema,
    margin: GridSpacingSchema,
    padding: GridSpacingSchema,
    autoColumnWidth: z.boolean().optional().describe("Whether columns should auto-size.")
  }).optional().describe("Layout configuration (functional grid layer)."),
  viewportWidth: z.number().optional().describe("Viewport width in pixels (used for breakpoint-aware suggestions)."),
  brand: z.enum(["hub", "water", "waste"]).optional().describe("DSS brand context."),
  theme: z.enum(["light", "dark"]).optional().describe("Theme context.")
});
